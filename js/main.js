...
/* ---------- Digitazione dell'h1 all'atterraggio ----------
   Una volta sola, non in ciclo. Il testo resta nel markup: se questo
   script non parte, l'h1 e' gia' completo e leggibile.
   L'h1 riceve un aria-label con il testo intero, cosi' il nome
   accessibile e' stabile e gli screen reader non leggono frammenti. */
(function () {
  'use strict';
  ...
  if (!span) return;
  var parte1 = (h.firstChild && h.firstChild.nodeValue ? h.firstChild.nodeValue : '').trim();
  var parte2 = span.textContent.trim();
  if (!parte1 || !parte2) return;
  var colore2 = span.style.color || '#17506b';

  /* riserva l'altezza misurata, cosi' svuotando l'h1 il contenuto
     sottostante non salta verso l'alto */
  h.style.minHeight = Math.ceil(h.getBoundingClientRect().height) + 'px';
  h.setAttribute('aria-label', parte1 + ' ' + parte2);
  ...
  wrap.appendChild(s1); wrap.appendChild(br); wrap.appendChild(s2);
  h.textContent = '';
  h.appendChild(wrap);
  h.classList.add('digita');

  var i = 0, tutto = parte1 + '\n' + parte2;
  function passo() {
    i++;
    var visto = tutto.slice(0, i);
    var taglio = visto.indexOf('\n');
    if (taglio === -1) { s1.textContent = visto; s2.textContent = ''; }
    else { s1.textContent = visto.slice(0, taglio); s2.textContent = visto.slice(taglio + 1); }
    if (i < tutto.length) { setTimeout(passo, 42); }
    else { h.classList.remove('digita'); h.style.minHeight = ''; }
  }
  setTimeout(passo, 450);
})();
